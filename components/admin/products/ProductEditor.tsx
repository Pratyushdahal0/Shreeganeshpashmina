...
import type { AdminProduct, ProductInput } from '@/lib/admin/products';
import { productCategories, productService } from '@/lib/admin/products';

type Props = { product?: AdminProduct };
const asInput = (product?: AdminProduct): ProductInput => ({ name: product?.name ?? '', slug: product?.slug ?? '', sku: product?.sku ?? '', description: product?.description ?? '', category: product?.category ?? '', price: product?.price, material: product?.material ?? '' });
